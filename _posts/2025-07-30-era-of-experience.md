...
### The Era of Experience Phases:

#### Synthetic-experience ramp-up

- Signals: Labs release more computer-use, Operator, Mariner style agents that click, code, and call APIs continuously
- Predictions: Compute spend shifts from one-shot inference to always-on interaction. GPU cost per trajectory becomes a new KPI

#### Reward diversification

- Signals: Vertical startups define composite metrics (e.g. reduced QA defects per deployment; lab assay yield per $, etc.)
- Predictions: Owning measurement pipelines (sensors, logs, BI dashboards) becomes more important than prompt engineering

#### World-model renaissance

- Signals: Papers on scalable planning in partially observed, non-stationary environments get popular in conferences
- Predictions: Toolchains for causal modelling, counterfactual simulation, and long-horizon credit assignment become the TensorFlow/PyTorch of the era

#### Safety loop issues

- Signals: Agents that can act for weeks raise new mis-alignment and interpretability worries, but also create richer feedback to correct them
- Predictions: Expect regulation that requires continuous auditing hooks, agent "flight recorders", and kill-switch APIs
...
This one is cool too. In the current paradigm, everyone's obsessed with prompt engineering. In the Era of Experience, the money will be in reward engineering.
...
- **Therapy companion agents** - Rewards based on validated mental health metrics  tracked over months, and not just "did the user like this response?" This requires deploying with real therapists, tracking real outcomes.
- **Code review agents** that optimize for "bugs caught before production" + "developer learning" + "team velocity" - a composite reward no generic coding assistant will target. FWIW, I think both Cursor and the Gemini team are working on this from what I've heard. 
...
Think Figma for the Era of Experience. Figma didn't invent vector graphics or collaboration. But they owned the interface where designers actually work. Now think "Figma for AI Agents" - dashboards where product managers can spin up agents, define reward functions with visual tools, monitor long-running agent workflows, and intervene when needed.
...
- **Developer environment agents** - Not another coding assistant, but something that lives in your IDE, watches your debugging patterns, learns your codebase's idioms. Cursor and Windsurf are early examples, but something like agents that learn from your entire team's workflow over months.
...
The next TensorFlow but for reward function composition. Something like Mixpanel + reinforcement learning. Let domain experts define complex objectives without coding. Think Airflow-grade orchestration for computing and updating rewards on live data streams.
